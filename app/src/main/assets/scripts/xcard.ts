...
class XCardGameEngine extends ModuleObject {
    protected version: string = "1.1.7";
    ...
    protected unofficialGame: boolean = false;
    ...
    public refreshPage(heavyRefresh: boolean = true) {
        ...
        if (this.needHideScreen) {
            ...
            this.refreshResult();
            if(this.unofficialGame) jq(this.placeArea).find('.span_finish_officials').removeClass('invisible');
            else jq(this.placeArea).find('.span_finish_officials').addClass('invisible');
    ...
    protected refreshMain() {
        ...
        var heightVal: number = window.innerHeight - 10; // jq(this.placeArea).height();
    ...
    protected refreshGame() {
        ...
        var currentPlayer: XCardPlayer = this.getPlayerNowTurn(); // 현재 차례의 플레이어
        ...
        var selfObj = this;
        hjow_runAfter(function () { // 적당한 딜레이 이후
    ...
    protected refreshResult() {
        var gameMode: XCardGameMode = this.getSelectedGameMode(); // 종료된 게임의 모드
        ...
        finResultLb.text('');
        if (this.resultReason != null) finResultLb.text(this.resultReason); // 라벨에 게임 종료 사유 입력
